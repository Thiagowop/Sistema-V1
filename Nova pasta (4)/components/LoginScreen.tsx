...
import { Lock, Mail, ArrowRight, Eye, EyeOff, Shield, Zap } from 'lucide-react';

interface LoginScreenProps {
  onLogin: () => void;
}

// Logo isolado com risco centralizado geometricamente
const BrandLogo = () => (
  <div className="flex flex-col items-center justify-center select-none">
    <div className="relative flex items-center justify-center py-2">
      ...
      {/* Linha Decorativa (Strikethrough) - Centralizada Absolutamente */}
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  useEffect(() => {
    const savedEmail = localStorage.getItem('mcsa_email');
    const savedPassword = localStorage.getItem('mcsa_password');
    const savedRemember = localStorage.getItem('mcsa_remember') === 'true';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (email && password) {
        if (rememberMe) {
          localStorage.setItem('mcsa_email', email);
          localStorage.setItem('mcsa_password', password);
          localStorage.setItem('mcsa_remember', 'true');
        } else {
          localStorage.removeItem('mcsa_email');
          localStorage.removeItem('mcsa_password');
          localStorage.removeItem('mcsa_remember');
        }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0f172a] relative overflow-hidden p-4">
      ...
      {/* Elementos decorativos animados de fundo (Refinados) */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {/* Gradiente base */}
        <div className="absolute inset-0 bg-gradient-to-b from-slate-900 via-[#0f172a] to-slate-900" />
        
        {/* Orbes de luz com animação 'blob' fluida e lenta */}
        <div 
          className="absolute top-[-10%] left-[-10%] w-[600px] h-[600px] bg-indigo-600/20 rounded-full blur-[120px] animate-blob mix-blend-screen" 
        />
        <div 
          className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-sky-600/15 rounded-full blur-[140px] animate-blob animation-delay-4000 mix-blend-screen" 
        />
        <div 
          className="absolute top-[40%] left-[60%] w-[300px] h-[300px] bg-violet-600/10 rounded-full blur-[100px] animate-blob animation-delay-8000 mix-blend-screen" 
        />
      </div>

      {/* Grid pattern overlay refinado e mais espaçado */}
