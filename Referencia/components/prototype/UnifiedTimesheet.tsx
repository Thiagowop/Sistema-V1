...
const formatDuration = (decimal: number, simple = false) => {
  ...
  if (simple) {
    // For heatmap tooltips or compact views where we want just the main number, 
    // maybe we stick to hours, but let's try to be consistent with "Xh" if space permits.
    // Given the prompt "never 0.2", we avoid raw decimals.
    return Math.round(decimal).toString() + 'h';
  }
  ...
  const renderCell = (log: DailyData, type: string, density: ViewDensity, isAgenda = false) => {
      ...
      if (density === 'heatmap') {
         return (
           <div className="flex flex-col items-center justify-center h-full w-full" title={`Real: ${realizedStr} / Meta: ${plannedStr}`}>
             <div className="text-[10px] font-bold text-slate-700">{formatDuration(log.realized, true)}</div>
    ...
    return (
      <div className={`w-full h-full flex items-center justify-center text-[10px] font-bold cursor-help transition-opacity hover:opacity-80 ${isOver ? 'bg-rose-100 text-rose-700' : `${intensity} text-slate-600`}`} title={`Planejado: ${plannedStr} | Realizado: ${realizedStr}`}>
         {Math.round(log.realized) > 0 ? formatDuration(log.realized, true) : <div className="w-1 h-1 rounded-full bg-slate-300"></div>}
      </div>
    );
  };
