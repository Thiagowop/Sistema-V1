...
import { ChevronLeft, ChevronRight, ChevronDown, ChevronRight as ChevronRightIcon, Calendar, Eye, EyeOff, Moon, Sun, LayoutGrid } from 'lucide-react';
import { GroupedData, AppConfig, Task as ClickUpTask } from '../types';
...
const TimesheetDashboard: React.FC<TimesheetDashboardProps> = ({ data, config }) => {
    ...
    // Transform ClickUp data to timesheet format
    const transformClickUpDataToTimesheet = useCallback((): Member[] => {
        ...
        data.forEach(group => {
            ...
            const member: Member = {
                ...
                name: group.assignee,
                initials: getInitials(group.assignee),
                projects: []
            };
            ...
            group.projects.forEach(project => {
                const transformedProject: Project = {
                    id: `${member.id}-${project.name.toLowerCase().replace(/\s+/g, '-')}`,
                    name: project.name,
                    tasks: []
                };
                ...
                project.tasks.forEach(task => {
                    const taskHours: Hours[] = allDays.map(dayData => {
                        const dateKey = dayData.date.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });

                        // Get hours from weeklyDistribution if available
                        const distributedHours = task.weeklyDistribution?.[dateKey];

                        if (distributedHours) {
                            // Parse the distributed hours string (e.g., "2h30m" or "4h")
                            const parseHoursString = (str: string): number => {
                            ...
                            const hours = parseHoursString(distributedHours);
                            return {
                                planned: task.timeEstimate > 0 ? hours : 0,
                                actual: task.timeLogged > 0 ? hours : 0
                            };
                        }

                        // No distribution for this day
                        return { planned: 0, actual: 0 };
                    });

                    transformedProject.tasks.push({
                        id: task.id,
                        name: task.name,
                        hours: taskHours
                    });
                ...
                if (transformedProject.tasks.length > 0) {
                    member.projects.push(transformedProject);
                }
            });
    ...
    const teamMembers = useMemo<Member[]>(() => transformClickUpDataToTimesheet(), [transformClickUpDataToTimesheet]);
                                        ...
                                        {day && (
                                            <>
                                                <div className="flex items-center justify-between mb-2">
                                                    ...
                                                    {totalActual > 0 && (
                                                        <span className={`text-xs px-1.5 py-0.5 rounded font-semibold ${totalActual > totalPlanned * 1.1
                                                ...
                                                <div className="space-y-1.5">
                                                    {projects.slice(0, 2).map((project, idx) => (
                                                        <div key={idx} className={`text-xs p-2 rounded border ${project.status === 'over'
                    ...
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={() => setShowWeekends(!showWeekends)}
                                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium ${showWeekends ? (isDark ? 'bg-gray-700 text-gray-200' : 'bg-gray-100 text-gray-700') : (isDark ? 'bg-blue-900 text-blue-200' : 'bg-blue-50 text-blue-700')
                                    }`}>
                    ...
                    <div className={`border-t ${isDark ? 'border-gray-700' : 'border-gray-200'} pt-4 mb-4`}>
                        <div className="flex gap-2">
                            <button onClick={() => setActiveTab('timeline')}
                                className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${activeTab === 'timeline' ? (isDark ? 'bg-gray-700 text-white' : 'bg-gray-900 text-white') : (isDark ? 'bg-gray-800 text-gray-400' : 'bg-gray-100 text-gray-700')
                                    }`}>
                                <LayoutGrid className="w-4 h-4" />
                                Timeline
                            </button>
                            <button onClick={() => setActiveTab('calendar')}
                                className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${activeTab === 'calendar' ? (isDark ? 'bg-gray-700 text-white' : 'bg-gray-900 text-white') : (isDark ? 'bg-gray-800 text-gray-400' : 'bg-gray-100 text-gray-700')
                                    }`}>
                    ...
                    <div className={`border-t ${isDark ? 'border-gray-700' : 'border-gray-200'} pt-4`}>
                        <div className="flex items-center gap-2 flex-wrap">
                            <button onClick={() => { setSelectedMemberFilter('all'); setExpandedProjects([]); }}
                                className={`px-4 py-2 rounded-lg text-sm font-medium ${selectedMemberFilter === 'all' ? (isDark ? 'bg-gray-700 text-white' : 'bg-gray-900 text-white') : (isDark ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-700')
                                    }`}>
                                Todos
                            </button>
                            {teamMembers.map(member => (
                                <button key={member.id} onClick={() => handleMemberFilterClick(member.id)}
                                    className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${selectedMemberFilter === member.id ? (isDark ? 'bg-gray-700 text-white' : 'bg-gray-900 text-white') : (isDark ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-700')
                                        }`}>
                                    <div className={`w-6 h-6 rounded flex items-center justify-center text-xs font-semibold ${selectedMemberFilter === member.id ? 'bg-white bg-opacity-20 text-white' : (isDark ? 'bg-gray-700 text-gray-300' : 'bg-white text-gray-700')
                                        }`}>{member.initials}</div>
                                    {member.name}
                                </button>
                            ))}
                                                ...
                                                return (
                                                    <div key={project.id} className={projIdx > 0 ? `border-t-2 ${isDark ? 'border-gray-700' : 'border-gray-200'}` : ''}>
                                                        <div onClick={() => toggleProject(project.id)}
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                {isExpanded ? <ChevronDown className={`w-4 h-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} /> : <ChevronRightIcon className={`w-4 h-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} />}
                                                        ...
                                                        {isExpanded && (
                                                            <div className={`${isDark ? 'bg-gray-850' : 'bg-gray-50'}`}>
                                                                {project.tasks.map((task, taskIdx) => (
                                                                    <div key={task.id} className={`h-16 px-6 ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-100'} transition-colors flex items-center ${taskIdx > 0 ? `border-t ${isDark ? 'border-gray-700' : 'border-gray-200'}` : 'border-t border-gray-200 dark:border-gray-700'
                                                                        }`}>
                                                        ...
                                                        <div key={project.id} className={projIdx > 0 ? `border-t-2 ${isDark ? 'border-gray-700' : 'border-gray-200'}` : ''}>
                                                            <div className={`h-20 flex ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
                                                                {days.map((day, dayIdx) => {
                                                                    ...
                                                                    return (
                                                                        <div key={dayIdx} className={`w-28 min-w-28 flex-shrink-0 border-r ${isDark ? 'border-gray-700' : 'border-gray-100'} p-2.5 flex items-center justify-center relative group ${day.isToday ? (isDark ? 'bg-blue-500/10 border-l-2 border-r-2 border-blue-600' : 'bg-blue-50 border-l-2 border-r-2 border-blue-300') :
                                                            ...
                                                            {isExpanded && project.tasks.map((task, taskIdx) => (
                                                                <div key={task.id} className={`h-16 flex ${isDark ? 'bg-gray-850' : 'bg-gray-50'} ${taskIdx > 0 ? `border-t ${isDark ? 'border-gray-700' : 'border-gray-200'}` : 'border-t border-gray-200 dark:border-gray-700'
                                                                    ...
                                                                    {days.map((day, dayIdx) => {
                                                                        ...
                                                                        return (
                                                                            <div key={dayIdx} className={`w-28 min-w-28 flex-shrink-0 border-r ${isDark ? 'border-gray-700' : 'border-gray-100'} p-2 flex items-center justify-center relative group ${day.isToday ? (isDark ? 'bg-blue-500/10 border-l-2 border-r-2 border-blue-600' : 'bg-blue-50 border-l-2 border-r-2 border-blue-300') :
