...
// Constants for Status Colors (matching Python script)
...
export const getDynamicWeekRange = (tasks: Task[]): Date[] => {
  ...
  // This ensures the week display is always up-to-date regardless of task dates
...
const getWeekFromDate = (startDate: Date): Date[] => {
  ...
  cursorDate.setHours(12, 0, 0, 0); // Noon to avoid DST issues

  let safetyLoop = 0;
  while (dates.length < 10 && safetyLoop < 30) {
    const day = cursorDate.getDay();

    // If not Saturday(6) or Sunday(0)
    if (day !== 0 && day !== 6) {
      ...
      complete: (results) => {
        try {
          ...
          rows.forEach((row, index) => {
            ...
            // Try to parse date closed if available in CSV (might be 'Date Closed' or 'Closed Date')
            ...
            const task: Task = {
              ...
              description: (row as any)['Description'] || (row as any)['Descrição'] || (row as any)['Descricao'] || undefined,
          ...
          // Determine Week Range First (using all tasks to find the "center of gravity")
          ...
          // Filter Active Tasks AND Recently Completed
          ...
          // Dynamic Grouping for CSV too
          const groupedTasksMap = new Map<string, Task[]>();

          activeTasks.forEach(t => {
            // Split the "Name / Name" string back into array
