...
import TimesheetSyncTab from '../../components/TimesheetSyncTab';
    ...
    <div className="flex flex-col h-full bg-[#F8FAFC] animate-fadeIn font-sans text-slate-800">
      ...
      <div className="bg-white px-6 py-4 border-b border-slate-200 shadow-sm flex flex-col xl:flex-row xl:items-center justify-between gap-4 flex-shrink-0 z-30">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${activeTab === 'dashboard' ? 'bg-indigo-600 text-white' : 'bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-100'}`} onClick={() => setActiveTab('dashboard')}>
            <Clock size={14} /> Dashboard
          </button>
          <button className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${activeTab === 'sync' ? 'bg-indigo-600 text-white' : 'bg-blue-50 text-blue-700 border border-blue-100 hover:bg-blue-100'}`} onClick={() => setActiveTab('sync')}>
            <Download size={14} /> Sincronizar
          </button>
        </div>
      </div>
      {/* Main Content */}
