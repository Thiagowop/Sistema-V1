...
const TimesheetDashboard: React.FC = () => {
  const { rawTasks } = useData(); // NEW: Get real data
  ...
  // NEW: Real team members from DataContext
  const teamMembers = useMemo<Member[]>(() => {
    ...
    rawTasks.forEach(task => {
      task.assignees?.forEach(assignee => {
        ...
        const member = membersMap.get(username)!;
        const projectName = task.list?.name || task.folder?.name || 'Sem Projeto';

        let project = member.projects.find(p => p.name === projectName);
        if (!project) {
          project = { id: `${username}-${projectName}`, name: projectName, tasks: [] };
          member.projects.push(project);
        }

        // Calculate hours from task time estimates/tracking
        const taskHours: Hours[] = allDays.map(() => {
          const planned = task.time_estimate
            ? (task.time_estimate / (1000 * 60 * 60)) / allDays.length
            : 0;
          const actual = task.time_spent
            ? (task.time_spent / (1000 * 60 * 60)) / allDays.length
            : 0;
          return {
  ...
  }, [rawTasks, allDays.length]);

  // Helper to sum hours across tasks
